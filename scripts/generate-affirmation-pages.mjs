...
const googleAnalyticsId = "G-5B0WGJZT0D";
...
function head({title, description, canonical, image, prefix = "../", detail = false, schema}) {
  return `<head>
    ...
    <script>if(location.hostname==="localhost"||location.hostname==="127.0.0.1")window["ga-disable-${googleAnalyticsId}"]=true;window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag("js",new Date());gtag("config","${googleAnalyticsId}");</script>
  </head>`;
}
...
const hub = `<!doctype html><html lang="en">${head({title:"Positive Affirmations for Confidence, Healing & Growth | Gratitude",description:hubDescription,canonical:"https://gratefulness.me/affirmations/",image:"https://www.gratefulness.me/images/affirmations-social-preview.png?v=2",schema:hubSchema})}<body class="affirmations-page">
  ...
  <main><section class="aff-hero" aria-labelledby="page-title"><div class="auto__container aff-hero__container"><div class="intro__inner"><div class="aff-hero__content intro__inner-content"><p class="aff-eyebrow"><span aria-hidden="true">✦</span> Your words shape your world</p><h1 id="page-title">Positive affirmations for <em>every part of you</em></h1><p class="aff-hero__intro">Choose what you need today. Find a little more confidence, make space for what matters, be gentle with yourself, and take things at your own pace.</p><a class="aff-primary" href="#affirmation-library">Explore affirmations <span aria-hidden="true">↓</span></a></div><div class="intro__inner-image"><img src="../images/ola.svg" alt="Woman using the Gratitude app" /><div class="intro__inner-image-plant"><img src="../images/plant.svg" alt="" /></div></div></div></div></section>
...
for (const category of allCategories) {
  ...
  const trackedQrUrl = `https://gratefulness.me/app/?utm_source=affirmations&utm_medium=qr&utm_campaign=app_download&utm_content=${encodeURIComponent(category.slug)}`;
  const storeBadges = `<div class="detail-store-buttons" role="group" aria-label="Download Gratitude"><a href="https://play.google.com/store/apps/details?id=com.northstar.gratitude" target="_blank" rel="noopener" aria-label="Get Gratitude on Google Play" data-app-event="google_play_click" data-app-platform="android" data-collection="${category.slug}"><img src="../../images/google-play.svg" alt="Get it on Google Play" width="154" height="46" /></a><a href="https://apps.apple.com/app/id1372575227" target="_blank" rel="noopener" aria-label="Download Gratitude on the App Store" data-app-event="app_store_click" data-app-platform="ios" data-collection="${category.slug}"><img src="../../images/app-store.svg" alt="Download on the App Store" width="154" height="46" /></a></div>`;
  const qrDownload = `<a class="detail-download-qr" href="${escapeHtml(trackedQrUrl)}" aria-label="Scan the QR code to download the Gratitude app"><span class="detail-download-qr__code" data-url="${escapeHtml(trackedQrUrl)}" aria-hidden="true"></span><span class="detail-download-qr__label"><strong>Scan to open</strong><span>Use your phone camera</span></span></a>`;
  const page = `<!doctype html><html lang="en">${head({title,description,canonical,image:category.blushImageURL,prefix:"../../",detail:true,schema})}<body class="affirmations-page detail-page"><a class="skip-link" href="#affirmation-list">Skip to affirmations</a>${header("../../")}<main><section class="detail-hero"><div class="auto__container"><nav class="detail-breadcrumb" aria-label="Breadcrumb"><a href="../">Affirmations</a><span aria-hidden="true">/</span><span>${escapeHtml(category.section.title)}</span></nav><div class="detail-hero__grid"><div class="detail-hero__copy"><h1>${escapeHtml(category.title)} <span>Affirmations</span></h1><p>${escapeHtml(category.section.description)}</p><div class="detail-app-panel"><div class="detail-app-copy"><div class="detail-app-message"><strong>Listen to all ${count} affirmations in Gratitude</strong><span>Open this collection in the app and choose a calming voice.</span></div><div class="detail-actions"><a class="aff-primary" href="${appLink(category)}">Open in Gratitude <span aria-hidden="true">↗</span></a>${storeBadges}<span class="detail-actions__divider" aria-hidden="true"></span>${qrDownload}</div></div></div></div><div class="detail-cover" style="--card-bg:${category.bgColor}">${category.blushImageURL ? `<img src="${category.blushImageURL}" alt="${escapeHtml(category.title)} affirmation collection artwork" />` : `<span aria-hidden="true">✦</span>`}<small>${count} affirmations</small></div></div></div></section><section class="detail-content" id="affirmation-list"><div class="auto__container detail-content__grid"><div><p class="aff-kicker">Read and repeat</p><h2>${count} affirmations for ${escapeHtml(category.title.toLowerCase())}</h2><p class="detail-intro">Pause after each sentence. Repeat it out loud or save the one you want to carry into your day.</p>${affirmationList}</div></div></section><section class="detail-related"><div class="auto__container"><div class="detail-related__heading"><div><p class="aff-kicker">Keep exploring</p><h2>More from ${escapeHtml(category.section.title)}</h2></div><a href="../">View all collections →</a></div><div class="detail-related__grid">${related}</div></div></section></main></div>${footer("../../")}<script defer src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script><script defer src="../../js/affirmation-detail.js?v=3"></script></body></html>`;
